"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Navigation = void 0;
var utils_1 = require("./utils");
var kendo_vue_common_1 = require("@progress/kendo-vue-common");
/**
 * @hidden
 */
var Navigation = /** @class */ (function () {
    function Navigation() {
    }
    Navigation.prototype.navigate = function (args) {
        var keyCode = args.keyCode;
        if (keyCode === kendo_vue_common_1.Keys.up || keyCode === kendo_vue_common_1.Keys.left) {
            return this.next({ current: args.current, min: args.min, max: args.max, step: -1 });
        }
        else if (keyCode === kendo_vue_common_1.Keys.down || keyCode === kendo_vue_common_1.Keys.right) {
            return this.next({ current: args.current, min: args.min, max: args.max, step: 1 });
        }
        else if (keyCode === kendo_vue_common_1.Keys.home) {
            return 0;
        }
        else if (keyCode === kendo_vue_common_1.Keys.end) {
            return args.max;
        }
    };
    Navigation.prototype.next = function (args) {
        if (!(0, utils_1.isPresent)(args.current)) {
            return args.min;
        }
        else {
            return Math.min(args.max, Math.max(args.current + args.step, args.min));
        }
    };
    return Navigation;
}());
exports.Navigation = Navigation;
